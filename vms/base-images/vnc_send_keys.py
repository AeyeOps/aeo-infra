#!/usr/bin/env python3
"""Extension of vnc_full.py: send a sequence of named keys (Up/Down/Enter/etc.)
...
def main():
    ...
    ap.add_argument("--keys", required=True,
                    help="Space-separated list of key names (Up, Down, Enter, ...)")
    ...
    for kname in args.keys.split():
        if kname not in KEYSYMS:
            print(f"[vnc] unknown key {kname!r}", file=sys.stderr)
            sys.exit(2)
        sym = KEYSYMS[kname]
        send_key(s, sym, True)
        time.sleep(hold_ms / 1000.0)
        send_key(s, sym, False)
        time.sleep(args.per_key_ms / 1000.0)
        print(f"[vnc] sent {kname}", file=sys.stderr)
