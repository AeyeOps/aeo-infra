...
You should see the other mesh nodes (myserver, mywsl, mywindows, etc.) and have a
...
- **SMB/Samba** works but requires Samba server configuration on myserver,
...
authorized on myserver.

```powershell
rclone config create myserver sftp host=100.64.0.10 user=myuser key_file=C:\Users\<username>\.ssh\id_ed25519 shell_type=unix
```
...
Register-ScheduledTask `
    ...
    -Principal $principal `
    -Settings $settings `
    -Description "Mount mesh shared directory from myserver via rclone SFTP"
```
...
- The cache lives at `%APPDATA%\rclone\vfs\myserver` and can be cleared with
  `rclone cache clear myserver:`
...
```powershell
ssh -i C:\Users\<username>\.ssh\id_ed25519 myuser@100.64.0.10 "echo ok"
```
...
----                    ------------    --        ----------------------
myserver                100.64.0.10      Linux     Local directory (NFS server)
mywsl (WSL2)       100.64.0.11      Linux     NFSv4 mount
mywindows (Windows)    100.64.0.12      Windows   rclone SFTP mount at C:\dev\shared
```
