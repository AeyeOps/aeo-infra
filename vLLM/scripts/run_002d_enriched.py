...
TRANSCRIPT_PATH = Path("/tmp/run_002d_transcript.md")
...
class TranscriptWriter:
    """Thread-safe markdown transcript writer. Appends after every turn, flushes on write.

    IO errors are caught and warned about — they never propagate into the worker.
    CSV is the authoritative artifact; transcript is a reviewer aid.
    """
    def __init__(self, path: Path):
        self.path = path
        self.lock = threading.Lock()
        self._fh = path.open("w")
        self._write_raw(f"# Run 002d — Conversation log\n\nStarted: {iso(time.time())}\n\n")
        self._sessions_written: set[int] = set()
    ...
    def write_turn(
        ...
        with self.lock:
            if session_id not in self._sessions_written:
                self._sessions_written.add(session_id)
                self._write_raw(f"\n## Session {session_id + 1} — {session_topic}\n\n")

            header = (
                f"### Turn {turn_index + 1} — {iso(time.time())} — "
                f"wall={wall:.1f}s ttft={ttft:.2f}s "
                f"prompt_tokens={prompt_tokens} completion_tokens={completion_tokens}\n\n"
            )
            self._write_raw(header)
            self._write_raw(f"**User:**\n{user_msg[:2000]}{'... [truncated]' if len(user_msg) > 2000 else ''}\n\n")
            if status == "ok":
                self._write_raw(f"**Assistant:**\n{assistant_msg}\n\n---\n\n")
            else:
                self._write_raw(f"**Status:** {status}\n\n---\n\n")
